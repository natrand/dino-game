...
Obstacles::Obstacles()
    : shape(sf::Vector2f(30, 50)), velocity(-300, 0), active(false), velocityX(0.0f), timeSinceLastSpawn(0.0f), spawnInterval(0.0f), obstacleSpacing(1.0f), player()

{
    if (obstaclesTexture.getSize().x == 0) {
        if (!obstaclesTexture.loadFromFile("Assets/cactus.png")) {
...
{
    active = true;
    shape.setPosition(600, floor.getPosition().y-20);
